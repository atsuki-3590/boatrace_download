...
# データcsv化
# 解凍したテキストファイルの格納先を指定
TEXT_FILE_DIR = "odds_data/odds_HTML_test"
...
def extract_odds(soup):
    ...
    for combination, (line, row) in odds_positions.items():
        odds_dict[combination] = get_odds(soup, line, row)
    return odds_dict

def get_data(text_file, race_code):
    with open(text_file, 'r', encoding='utf-8') as file:
        soup = BeautifulSoup(file, 'html.parser')
        odds_data = extract_odds(soup)
        # データの抽出
...
def main():
    ...
    for file_name in os.listdir(TEXT_FILE_DIR):
        if file_name.endswith(".html"):
            ...
            data = get_data(file_path, race_code)
